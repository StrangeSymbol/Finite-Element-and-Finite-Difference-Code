...
for m = 1:count
    ...
    if ((i+1) * dx)^2 + (j * dy)^2 < 1 && ((i-1) * dx)^2 + (j * dy)^2 < 1 
        ...
        if ((i+1) * dx)^2 + (j * dy)^2 > 1
            ...
            A(m, getIndex(P,i,j,count)) =  -2 / (tau * dx^2);
            A(m, getIndex(P,i-1,j,count)) = 2 / ((tau + 1) * dx^2);
            temp = temp + 2 / ((tau * (1+tau)) * dx^2);
        ...
        if ((i-1) * dx)^2 + (j * dy)^2 > 1
            tau = abs(-sqrt(1 - (j*dy)^2) - i*dx) / dx;
            A(m, getIndex(P,i,j,count)) = A(m, getIndex(P,i,j,count)) - 2 / (tau * dx^2);
            A(m, getIndex(P,i+1,j,count)) = 2 / ((tau + 1) * dx^2);
            temp = temp + 2 / ((tau * (1+tau)) * dx^2);
        elseif ((i-1) * dx)^2 + (j * dy)^2 == 1.0
            A(m, getIndex(P,i+1,j,count)) = C(1);
    ...
    if (i * dx)^2 + ((j+1) * dy)^2 < 1 && (i * dx)^2 + ((j-1) * dy)^2 < 1
        ...
        if (i * dx)^2 + ((j+1) * dy)^2 > 1
            ...
            A(m, getIndex(P,i,j,count)) = A(m, getIndex(P,i,j,count)) - 2 / (tau * dy^2);
            A(m, getIndex(P,i,j-1,count)) = 2 / ((tau + 1) * dy^2);
            temp = temp + 2 / ((tau * (1+tau)) * dy^2);
